...
interface ChatbotPanelProps {
  query: string; 
  setQuery: (value: string) => void; // Update query state
  answer: any; // Current chatbot resposne
  allSchemes: any[];
  currentScheme: any; // Current displayed scheme
  carouselIndex: number;
  setCarouselIndex: React.Dispatch<React.SetStateAction<number>>; // Function to update carousel position
  renderChatbotContent: (res: any) => React.ReactNode;
  fetchResponse: () => void; // Function to trigger API call
  isRecording: boolean;
  startRecording: () => void;
  stopRecording: () => void;
  playAnswer: () => void;
...
const ChatbotPanel: React.FC<ChatbotPanelProps> = ({
  ...
  return (
    <div className="flex flex-col md:flex-row mt-6 gap-6">
      ...
      <div className="flex flex-col items-center w-full md:w-1/3 gap-4 bg-[#c9e2d6] p-5 md:p-8 rounded-2xl shadow relative">
        ...
        <div
          ...
          className={`w-24 h-24 rounded-full flex items-center justify-center shadow cursor-pointer transition-transform ${
            isRecording ? "bg-red-400 animate-pulse" : "bg-[#dce8e1]" // Red pulsing when recording
          }`}
          onClick={isRecording ? stopRecording : startRecording} // Toggle recording state
        >
        ...
        {/* Text area container for chatbot and search button */}
        <div className="relative w-full">
          ...
          />
          {/* Search button positioned in bottom-right of textarea */}
          <button
        ...
        {/* Suggested prompts, with decreasing opacity */}
        {["What is CTG?", "Government Grants", "Dementia Resources"].map((prompt, idx) => (
          ...
          >
            <div
              onClick={() => setQuery(prompt)} // Set query to suggested prompt when clicked, but still need to click search button fyi
      ...
      {/* Right Section for chatbot response display */}
      <div className="flex-1 bg-[#c9e2d6] rounded-2xl p-5 md:p-8 shadow relative">
        {/* Carousel (when there are multiple outputs or schemes) */}
        <div className="flex justify-between items-center">
          ...
          {/* Dynamic title of current scheme */}
        ...
        <div className="relative mt-4">
          <div className="relative bg-white overflow-visible p-4 py-6 rounded-lg text-sm space-y-4 min-h-[150px] flex-grow h-full shadow-md speech-bubble">
            ...
            {/* Conditional content based on loading and answer state */}
            {loading ? (
            ...
            ) : answer ? ( // Render content or show no schemes messages
